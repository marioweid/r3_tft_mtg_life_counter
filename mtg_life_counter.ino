...
// enum PanelMode : uint8_t { MODE_LIFE=0, MODE_POISON=1, MODE_CMDMG=2 };
...
//   tft.print("P"); tft.print(p+1);
//   // mode tag
//   tft.setCursor(x+28, y+4);
//   switch (modeP[p]) {
//     case MODE_LIFE:   tft.print(" Life");   break;
//     case MODE_POISON: tft.print(" Poison"); break;
//     case MODE_CMDMG:  tft.print(" Cmd Dmg"); break;
//   }
// }
...
//   int16_t numX = x + 48;
//   int16_t numY = y + 36;
//   if (lifeT[p] >= 100 || lifeT[p] <= -10) numX = x + 28; // crude centering
...
//     tft.print("P"); tft.print(attacker+1); tft.print(">");
...
//   if (modeP[p] == MODE_LIFE) drawPanelLife(p);
// }

// void updatePoison(uint8_t p) {
//   if (modeP[p] == MODE_POISON) drawPanelPoison(p);
// }

// void updateCmdmg(uint8_t p) {
//   if (modeP[p] == MODE_CMDMG) drawPanelCmdmg(p);
// }
...
void setup() {
  ...
  // Cross lines to visually separate panels
  tft.drawLine(0, 0, 20, 20, GRAY);
  tft.drawLine(0, 0, 520, 240, GRAY);
  currentcolor = RED;
  pinMode(13, OUTPUT);
...
// /************ Touch mapping for rotation=1 ************/
// /* If touches feel flipped, swap the map() directions or swap x/y. */
// void readTouch(int16_t &x, int16_t &y, int16_t &z) {
//   TSPoint p = ts.getPoint();
//   // Re-enable shared pins for TFT after reading
//   pinMode(XM, OUTPUT);
//   pinMode(YP, OUTPUT);

//   z = p.z;
//   if (z > MINPRESSURE && z < MAXPRESSURE) {
//     // For rotation=1, these mappings are typical on Elegoo 2.8" ILI9341
...
void loop() {
  ...
  if (p.z > MINPRESSURE && p.z < MAXPRESSURE) {
    ...
    */
    ...
    // y = (tft.height()-map(p.y, TS_MINY, TS_MAXY, tft.width(), 0));
    ...
    Serial.print("Mapped 2 X = "); Serial.print(x);
    Serial.print("\tMapped 2 Y = "); Serial.println(y);
  ...
  // if (z <= MINPRESSURE || z >= MAXPRESSURE) return;
  ...
  //     lifeT[player] = 40;
  //     poisonT[player] = 0;
  //     for (uint8_t a=0;a<NUM_PLAYERS;a++) cmdmg[player][a] = 0;
  //   }
  //   drawAll();
  //   // delay(250);
  //   return;
  ...
  //   if (inRect(x, y, r)) { panel = player; break; }
  // }
  // if (panel == 0xFF) return; // outside
  ...
  //   if (inRect(x,y, minus)) { lifeT[panel]--; updateLife(panel); delay(150); return; }
  //   if (inRect(x,y, plus))  { lifeT[panel]++; updateLife(panel); delay(150); return; }
  // }
  // else if (modeP[panel] == MODE_POISON) {
  //   Rect minus = { x0+8,   y0+72, 44, 36 };
  //   Rect plus  = { x0+108, y0+72, 44, 36 };
  //   if (inRect(x,y, minus)) { if (poisonT[panel] > 0) poisonT[panel]--; updatePoison(panel); delay(150); return; }
  //   if (inRect(x,y, plus))  { if (poisonT[panel] < 10) poisonT[panel]++; updatePoison(panel); delay(150); return; }
  // }
  // else if (modeP[panel] == MODE_CMDMG) {
  //   // Identify which attacker row: three rows skipping self
  //   uint8_t order[3]; uint8_t idx=0;
  //   for (uint8_t a=0; a<NUM_PLAYERS; a++) if (a!=panel) order[idx++]=a;
  ...
  //     if (inRect(x,y, minus)) { if (cmdmg[panel][attacker] > 0) cmdmg[panel][attacker]--; updateCmdmg(panel); delay(140); return; }
  //     if (inRect(x,y, plus))  { if (cmdmg[panel][attacker] < 21) cmdmg[panel][attacker]++; updateCmdmg(panel); delay(140); return; }
